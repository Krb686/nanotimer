...
-Timer objects use the non-blocking feature 'setImmediate' for time counting and synchronization.

-Errors in timing are also non-cumulative.  For example, when using the setInterval command, the timer counts
...
* Repeatedly calls function 'task' after every interval amount of time.
