...
NanoTimer.prototype.setInterval = function(task, interval, callback){
    ...
    if(intervalType == 's'){
        ...
        console.log('Error: Incorrect interval format. Format is an integer followed by "s" for seconds, "m" for milli, "u" for micro, and "n" for nanoseconds. Ex. 2u');
        process.exit(1);
    }
    ...
    if(intervalTime > 0){
        ...
        //Check for overflow.  Every 100,000 seconds (1666 minutes or 28ish hours, this will overflow
        //and the reference time T1 will be re-acquired.  This is the only case in which error will 
        //propagate because error is only added due to the amount of time needed to complete this action.
        if(intervalTime*this.intervalCount > 100000000000000){
            this.intervalT1 = process.hrtime();
            this.intervalCount = 0;
        }
        ...
        if(this.intervalContinue == true){
            if(this.difTime < (intervalTime*this.intervalCount)){
                setImmediate(function(){thisTimer.setInterval(task, interval, callback);});
            }
            else{
                task();
                this.intervalCount++;
...
NanoTimer.prototype.setTimeout = function(task, delay, callback){
    ...
    if(delayType == 's'){
        ...
        console.log('Error: Incorrect delay format. Format is an integer followed by "s" for seconds, "m" for milli, "u" for micro, and "n" for nanoseconds. Ex. 2u');
        process.exit(1);
    }
    ...
    if(difTime < delayTime){
        setImmediate(function(){thisTimer.setTimeout(task, delay, callback);});
    }
    else{   //time to run
